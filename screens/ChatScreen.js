  ...
  useEffect(() => {
    (async () => {
      ...
      if (msgs.length === 0) {
        ...
        if (storedName) {
          welcomeText = `${storedName}님, 반갑습니다. 지금 고등학생이신가요, 대학생이신가요? 몇 년생이신지 또는 회사에 다니고 계신지, 사업을 준비 중이신지 알려주시면 어떻게 도와드릴지 파악하는 데 도움이 될 것 같아요.`;
        }
  ...
  const getAIResponse = async (text) => {
    try {
      ...
      const systemPrompt = `
        너는 사용자의 친근한 전문 코치야.
        항상 정중한 "~요" 체로 이야기해 줘.
        답변은 다음과 같은 방식으로 작성해 줘:

        1) 먼저 짧게 격려나 맞장구로 시작한다. (예: "와, 멋진 목표네요!", "좋아요, 한번 해보죠!")
